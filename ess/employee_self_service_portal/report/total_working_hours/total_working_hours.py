...
def get_data(filters=None):
    ...
    attendance = frappe.db.sql('''select
                                        ...
                                        date_format(out_time, '%H:%i:%s') as 'out_time'
                                        from `tabAttendance` where docstatus =1 {0} order by attendance_date DESC'''.format(conditions),as_dict=True)
    # attendance = frappe.db.sql('''select attendance_date, shift,employee, check_in_date_time, out_time, late_entry, early_exit from `tabAttendance` where docstatus =1 {0}'''.format(conditions),as_dict=True)
...
def update_shift_details(attendance_dict):
    ...
    checkin_time = attendance_dict.get('in_time')
    checkout_time = attendance_dict.get('out_time')
