...
from erpnext.setup.doctype.employee.employee import deactivate_sales_person, is_holiday
import frappe
...
@frappe.whitelist()
def checkout_creation(data):
	# frappe.error_log("checkout",str(data))
	import json
	if isinstance(data, str):
		data = json.loads(data)
	data = frappe._dict(data)
	...
	eid = frappe.db.sql("""select name,employee_name,company from `tabEmployee` where user_id = %s""",(frappe.session.user),as_dict=1)
	if eid:
		emp_check_in.employee = eid[0].name
		emp_check_in.employee_name = eid[0].employee_name
		emp_check_in.company = eid[0].company
		emp_check_in.log_type = "OUT"
		# emp_check_in.check_infrom = checkin_from
		from datetime import datetime
		# datetime object containing current date and time
		now = datetime.now()
